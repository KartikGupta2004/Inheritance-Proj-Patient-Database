...
function LabTest() {
  ...
  useEffect(() => {
    ...
    // Example fetch call
    fetch('http://localhost:5000/api/lab_test/addlabtest')
      .then((response) => response.json())
      .then((data) => setLabtest(data))
      .catch((error) => console.error('Error fetching details:', error));
  }, []);

  const handleDelete = (labtestId) => {
    // Implement delete functionality here
    // Example delete call
    fetch(`http://localhost:5000/api/lab_test/addlabtest/${labtestId}`, {
      method: 'DELETE',
    })
      .then(() => {
        // Update visits state after successful delete
        setLabtest(labtest.filter((labtest) => labtest.id !== labtestId));
      })
  ...
  return (
    ...
    <div className='text-2xl sm:text-4xl lg:text-5xl space-y-2 '>
        <img className='w-40 lg:w-72 flex mx-auto justify-center' src="Medical page/result.jpg" alt="" />
    ...
    <div key={labtest.id} className='flex justify-center items-center'>
  <div className='w-4/5 text-center '>
    <div className='flex h-fit justify-between items-center p-3 md:p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700'>
      ...
      <div className='mb-0 text-md md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
        ...
        <p className='mb-0 text-gray-400'>{labtest.date}</p>
        </div>
      </div>
      </div>
    ...
    {selectedOption === 'Details' && (
      <div className='block bg-gray-100 border-t-2 border-y-yellow-100 pt-5  text-2xl rounded-lg'>
        <div className='flex flex-col justify-center items-center'>
        <p className='text-gray-500'>Test: <span className='text-black'>{labtest.test}</span> </p>
        <p className='text-gray-500'>Result: <span className='text-black'>{labtest-result}</span></p>
        <p className='text-gray-500'>Normal: <span className='text-black'>{labtest.normal}</span></p>
        <p className='flex ml-4 justify-start text-gray-500'>Doctor:<span className='ml-3 text-black'>{labtest.doctor}</span></p>
