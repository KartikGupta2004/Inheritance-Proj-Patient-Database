...
const Notes=require('../models/vaccine.js')
...
router.post('/addvaccine',fetchuser ,async(req,res)=>{
    try{
...
router.put('/updatevaccine/:id',fetchuser,async(req,res)=>{
    const {title,date}=req.body;
...
router.delete('/deletevaccine/:id',fetchuser,async(req,res)=>{
   const {title,date}=req.body;
    // Create a newNote object
    const newNote={}
    if(title){newNote.title=title}
