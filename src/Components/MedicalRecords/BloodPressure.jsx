...
function BloodPressure() {
  ...
  useEffect(() => {
    ...
    // Example fetch call
    fetch('http://localhost:5000/api/bp/addbp')
      .then((response) => response.json())
      .then((data) => setBp(data))
      .catch((error) => console.error('Error fetching data:', error));
  }, []);

  const handleDelete = (bpId) => {
    // Implement delete functionality here
    // Example delete call
    fetch(`http://localhost:5000/api/bp/addbp/${bpId}`, {
      method: 'DELETE',
    })
      .then(() => {
        // Update visits state after successful delete
        setBp(bp.filter((bp) => bp.id !== bpId));
      })
  ...
  return (
    ...
    <div className='text-2xl sm:text-4xl lg:text-5xl space-y-2 '>
        <img className='w-40 lg:w-72 flex mx-auto justify-center' src="Medical page/blood-pressure.jpg" alt="" />
        <p className='font-bold'>No Data</p>
        <div className='text-gray-400'>
        <p >Click Add Button and</p>
        <p>Enter Blood Pressure Data</p>
        </div>
    </div>
    )
    :(bp.map((bp)=>(
    <div key={bp.id} className='flex justify-center items-center'>
    ...
    <div className='flex justify-between'>
      <img className='w-12 h-12 md:w-16 md:h-16 lg:w-24 lg:h-24 bg-gray-200 lg:p-2 rounded-lg flex' src="Photos/bloodpressure.png" alt="" />
      <div className='flex'>
      ...
      <div className='mb-0 text-lg md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
        <table>
          <tr>
            <td className='pr-1 sm:pr-3 md:pr-16'>Systolic</td>
            <td className=' sm:px-3 md:px-16'>Diastolic</td>
            <td className='pl-1 sm:pl-3 md:pl-16'>Pulse</td>
          </tr>
          <tr>
            <td className='text-gray-400 border-r-2 border-yellow-500 md:pr-16 sm:pt-2'>{bp.systolic}</td>
            <td className='text-gray-400 border-r-2 border-yellow-500 sm:pt-2 md:px-16 '>{bp.diastolic}</td>
            <td className='text-gray-400 sm:pt-2 md:pl-16'>{bp.pulse}</td>
          </tr>
        </table>
        <div className='flex mt-2 '>
        <p className='mb-0 mr-3 md:mr-40'>{bp.date}</p>
        <p className='mb-0'>{bp.time}</p>
        </div>
        </div>
      </div>
      </div>
    ...
    <div className='m-10 text-white bg-red-500 text-6xl w-15 lg:text-8xl lg:w-24 rounded-full hover:cursor-pointer'>
