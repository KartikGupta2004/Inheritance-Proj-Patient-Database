...
function BloodPressure() {
  ...
  const getData = async () => {
    try {
      const response = await fetch(
        "http://localhost:5000/api/blood_pressure/fetchallpressurerec",
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/blood_pressure/deletepressurerec/${id}`,
        {
          method: "DELETE",
          headers: {
            token: user.authToken,
      ...
      <div className='bg-fuchsia-50 h-fit text-center mx-auto'>
              ...
              {data.length === 0 ? (
                <div className='text-2xl sm:text-4xl lg:text-5xl space-y-2 '>
                  <img
                    ...
                    src='Medical page/blood-pressure.jpg'
                    alt=''
                  />
                ...
                data.map((bp) => (
                  ...
                  >
                    <div className='w-11/12 h-fit p-3 md:p-4 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700'>
                      <div className='flex justify-between'>
                        <img
                          className='w-12 h-12 md:w-16 md:h-16 lg:w-24 lg:h-24 bg-gray-200 lg:p-2 rounded-lg flex'
                          src='Photos/bloodpressure.png'
                          alt=''
                        />
                        <div className='flex'>
                          <div className='flex items-center ml-6 no-underline'>
                            <div className='mb-0 text-lg md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
                              <table>
                                ...
                                <tr>
                                  <td className='text-gray-400 border-r-2 border-yellow-500 md:pr-16 sm:pt-2'>
                                    {bp.systolic}
                                  </td>
                                  <td className='text-gray-400 border-r-2 border-yellow-500 sm:pt-2 md:px-16 '>
                                    {bp.diastolic}
                                  </td>
                                  <td className='text-gray-400 sm:pt-2 md:pl-16'>
                                    {bp.pulse}
                                  </td>
                                </tr>
                              </table>
                        ...
                        <div className='xl:ml-20 justify-end'>
                          <div className='flex mt-2 mb-3 md:ml-7 text-gray-500 text-xl md:text-3xl'>
                            <BsThreeDotsVertical
                              className='hover:cursor-pointer'
                              onClick={() => toggleDropdown(bp._id)}
                            />
                            {isDropdownVisible[bp._id] && (
        ...
        <div className=' flex justify-end items-end mx-auto'>
          <div className='m-10 text-white bg-red-500 text-6xl w-15 lg:text-8xl lg:w-24 rounded-full hover:cursor-pointer'>
            <Link className='text-white' to='/addbloodpressure'>
              <LuPlus />
            </Link>
          </div>
        </div>
