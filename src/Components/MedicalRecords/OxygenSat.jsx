...
function OxygenSat() {
  ...
  useEffect(() => {
    ...
    // Example fetch call
    fetch('http://localhost:5000/api/os/addos')
  ...
  const handleDelete = (osId) => {
    ...
    // Example delete call
    fetch(`http://localhost:5000/api/os/addos/${osId}`, {
      method: 'DELETE',
    })
      .then(() => {
        // Update visits state after successful delete
        setos(os.filter((os) => os.id !== osId));
      })
  ...
  return (
    ...
    <div className='text-2xl sm:text-4xl lg:text-5xl space-y-2 '>
        <img className='w-40 lg:w-72 flex mx-auto justify-center' src="Medical page/oxygen-saturation.jpg" alt="" />
    ...
    <div key={os.id} className='flex justify-center items-center'>
    ...
    <div className='flex justify-between'>
      <img className='w-12 h-12 md:w-16 md:h-16 lg:w-24 lg:h-24 bg-gray-200 lg:p-2 rounded-lg flex' src="Photos/oxygen-saturation.png" alt="" />
      <div className='flex'>
      ...
      <div className='mb-0 text-lg md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
        <table>
          <tr>
            <td className='pr-1 sm:pr-3 md:pr-16'>Result</td>
            <td className='pl-1 sm:pl-3 md:pl-16'>Pulse</td>
          </tr>
          <tr>
            <td className='text-gray-400 border-r-2 border-yellow-500 sm:pt-2 pt-1 pr-2 sm:pr-16'>{os.result} mg/dL</td>
            <td className='text-gray-400 pt-1 sm:pt-2 pl-2 sm:pl-16'>{os.pulse}</td>
          </tr>
        </table>
