...
function BloodGlucose() {
  ...
  useEffect(() => {
    ...
    // Example fetch call
    fetch('http://localhost:5000/api/bg/addbg')
      .then((response) => response.json())
      .then((data) => setBg(data))
      .catch((error) => console.error('Error fetching data:', error));
  }, []);

  const handleDelete = (bgId) => {
    // Implement delete functionality here
    // Example delete call
    fetch(`http://localhost:5000/api/bg/addbg/${bgId}`, {
      method: 'DELETE',
    })
      .then(() => {
        // Update visits state after successful delete
        setBg(bg.filter((bg) => bg.id !== bgId));
      })
  ...
  return (
    ...
    <div className='flex justify-between'>
      <img className='w-12 h-12 md:w-16 md:h-16 lg:w-24 lg:h-24 bg-gray-200 lg:p-2 rounded-lg flex' src="Photos/bloodglucose.png" alt="" />
      <div className='flex'>
      ...
      <div className='mb-0 text-lg md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
        <table>
          <tr>
            ...
            <td className='pl-1 sm:pl-3 md:pl-16'>Type</td>
          </tr>
          <tr>
            <td className='text-gray-400 border-r-2 border-yellow-500 sm:pt-2 pt-1 pr-2 sm:pr-16'>{bg.result} mg/dL</td>
            <td className='text-gray-400 pt-1 sm:pt-2 pl-2 sm:pl-16'>{bg.type}</td>
          </tr>
        </table>
        <div className='flex mt-2 '>
        <p className='mb-0 mr-3 md:mr-40'>{bg.date}</p>
        <p className='mb-0'>{bg.time}</p>
        </div>
        </div>
      </div>
      </div>
