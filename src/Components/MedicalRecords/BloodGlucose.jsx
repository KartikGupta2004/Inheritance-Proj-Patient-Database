...
function BloodGlucose() {
  ...
  const getData = async () => {
    try {
      const response = await fetch(
        "http://localhost:5000/api/blood_glucose/fetchallbloodrec",
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/blood_glucose/deletebloodrec/${id}`,
        {
          method: "DELETE",
          headers: {
            token: user.authToken,
      ...
      <div className='bg-fuchsia-50 h-fit text-center mx-auto'>
              ...
              {data.length === 0 ? (
                <div className='text-2xl sm:text-4xl lg:text-5xl space-y-2 '>
                  <img
                    ...
                    src='Medical page/glucose-meter.jpg'
                    alt=''
                  />
                            ...
                            <div className='mb-0 text-lg md:text-2xl lg:text-3xl font-medium tracking-tight text-gray-900 dark:text-white'>
                              <table>
                                ...
                                <tr>
                                  ...
                                  <td className='text-gray-400 pt-1 sm:pt-2 pl-2 sm:pl-16 capitalize'>
                                    {val.rec_type}
                                  </td>
                                </tr>
                              </table>
                              <div className='flex mt-2 '>
                                <p className='mb-0 mr-3 md:mr-40'>{val.date}</p>
                                <p className='mb-0'>{val.time}</p>
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        <div className=' flex justify-end items-end mx-auto'>
          <div className='m-10 text-white bg-red-500 text-6xl w-16 lg:text-8xl lg:w-24 rounded-full hover:cursor-pointer'>
            <Link className='text-white' to='/addbloodglucose'>
              <LuPlus />
            </Link>
          </div>
        </div>
